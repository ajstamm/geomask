...
For example, a flag value of 0 means the first buffer was valid. A flag of 2 means the buffer had to be recalculated twice to create a valid area.

To map these files yourself after the geomasker finishes, you can use the following code. The first part reads in the shapefiles. The second part draws the map.
...
In this step, nothing happens if you are following the vignette and selected "No" when asked if you wanted to save a KML file. If you had selected "Yes", a KML file and a KMZ (zipped KML) file would be created in this step that could then be opened in Google Earth. 
